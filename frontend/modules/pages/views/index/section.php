...
<section class="text-right clear">
    ...
    <p class="annotate"><?= StringHelper::truncate($item['short_description_'. $language], 200)?></p>
        <?php if(!is_null($program)):?>
            ...
            <?php if($item->is_announcement == 1):?>
                <?php if(!is_null($item->announcement_date)):?>
                    <span class="time"><?= $item->formatAnnouncementDate(); ?></span>
                <?php endif;?>
            <?php else:?>
        ...
        <?php else:?>
            <?php if($type): ?>
                ...
                <?php else:?>
                    <?php if($item->is_announcement == 1):?>
                        <?php if(!is_null($item->announcement_date)):?>
                            <span class="time main-list"><?= $item->formatAnnouncementDate(); ?></span>
                        <?php endif;?>
                    <?php else:?>
