...
<section class="current-program-wrap light">
    <?php if($language =='ru'):?>
        <h1>Последние выпуски</h1>
    <?php else:?>
        <h1>Останні випуски</h1>
    <?php endif;?>
        ...
        <?php foreach ($news as $item):?>
            ...
            <div class="preview">
                <?php if($item->type == 'text'): ?>
                    <?php if($preview = Preview::find()->where(['news_id'=> $item->id])->one()):?>
                ...
                <?php else: ?>
                    <?php if($video = Video::find()->where(['news_id' => $item->id])->one()):?>
